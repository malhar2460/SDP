...
function App() {
  ...
  //     // setdate(formatDate(new Date(arrivalTime)))
  //     // settime(new Date(arrivalTime).toLocaleTimeString())
  //     setDuration(durationInSeconds)
  //     axios.post('http://127.0.0.1:8000/data/', {'duration': durationInSeconds,"date":formatDate(new Date(arrivalTime)) ,"time":new Date(arrivalTime).toLocaleTimeString() }, { headers: { 'Content-Type': 'application/json' } })
  //       .then((response) => {
  ...
  const handleGuestIdUpdate = (guestIdFromRecognition) => {
    ...
    const updateDurationOnServer = () => {
      ...
      //   if (performance.navigation.type != 1) {
        const isPageReloaded = window.performance && performance.navigation.type === 1;
        if(!isPageReloaded)
        {
          axios.post('http://127.0.0.1:8000/data/', {'user_id':localStorage.getItem('user'),'guest_id':localStorage.getItem('guest_id'), 'duration': durationInSeconds,"date":formatDate(new Date(arrivalTime)),"time": new Date(arrivalTime).toLocaleTimeString()}, { headers: { 'Content-Type': 'application/json' } })
              .then((response) => {
                console.log(response.data);
      ...
      //       console.log("Record already exists, updating the duration...");
      //       // If the record exists, update the duration instead of creating a new record
      //       axios.patch('http://127.0.0.1:8000/data/', {
      ...
      //         });
      //     } else {
      //       console.log("Record does not exist, creating a new record...");
      ...
      //         })
      //         .catch((error) => {
      //           console.error("Error while creating a new record on the server", error);
      //         });
      //     }
      //   })
      //   .catch((error) => {
      //     console.error("Error while checking record existence on the server", error);
      //   });
    };
  ...
  return (
    <div className="App">
      ...
      <Router>
        <Routes>
          <Route path="/" element={<HandGestureRecognition onGuestIdUpdate={handleGuestIdUpdate}/>}></Route>
